...
using namespace std;
inline static const vector<int> get_neighbours(unsigned int x, unsigned int y,
...
two_pass(std::vector<std::vector<uint8_t>> data) {
  std::vector<std::vector<int>> labels(data.size(),
                                       std::vector<int>(data[0].size(), 0));
  unsigned int *rank = new unsigned int[data.size() * (data[0].size())];
  unsigned int *parent = new unsigned int[data.size() * (data[0].size())];
  vector<vector<point>> groups(data.size() * (data[0].size() ),
                               vector<point>());
